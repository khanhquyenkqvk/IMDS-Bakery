...
function initSearchAndFilters() {
    ...
    const filterable = () => Array.from(document.querySelectorAll('.guide-card, .faq-item'));
    let activeTopic = 'all';
    ...
    const apply = () => {
        ...
        filterable().forEach((el) => {
            ...
            el.classList.toggle('is-hidden', !(matchesTopic && matchesQuery));
        });
    };
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                apply();
                const firstMatch = document.querySelector('.guide-card:not(.is-hidden), .faq-item:not(.is-hidden)');
...
function initContactActions() {
    const actionable = document.querySelectorAll('[data-action]');
